...
interface Props {
  ...
  initialMentors: MentorCardData[];
}

export default function MentoringClient({ isMentor, existingMentorId, initialMentors }: Props) {
  const [specialty, setSpecialty] = useState('');
  const [state, setState]         = useState('');
  const [mode, setMode]           = useState('');

  const mentors = useMemo(() => {
    return initialMentors.filter(m => {
  ...
  }, [initialMentors, specialty, state, mode]);
